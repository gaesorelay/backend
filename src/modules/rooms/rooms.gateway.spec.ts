...
describe('RoomsGateway.submit_vote', () => {
  ...
  it('rejects votes when room is not in VOTING state', async () => {
    // VOTING 상태가 아닐 때 투표가 차단되는지 확인
    roomsService.getRoomById.mockResolvedValue({ status: 'PLAYING' });

    const result = await gateway.handleSubmitVote(socket, { team: 'A' });

    expect(result).toEqual({ status: 'error', message: 'Voting is not open.' });
    expect(gamesService.submitAudienceVote).not.toHaveBeenCalled();
    expect(emitSpy).not.toHaveBeenCalled();
  });

  it('accepts votes when room is in VOTING state', async () => {
    // VOTING 상태일 때 투표가 수락되고 브로드캐스트되는지 확인
    roomsService.getRoomById.mockResolvedValue({ status: 'VOTING' });
